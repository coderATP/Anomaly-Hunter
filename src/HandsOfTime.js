export class HandsOfTime{
    ...
    createDeck(){
        ...
        for(let i = 0; i < HandsOfTime.CARD_SUITS.length; ++i ){
            
            const startFrame = Object.values(HandsOfTime.CARD_START_FRAMES)[i];
            for(let j = 0; j < 13; ++j){
               const card = this.scene.createCard("null", this.config.width-120, 0, true)
                   ...
                   .setData({
                       frame: startFrame + j,
                       value: j+1,
                       suit: HandsOfTime.CARD_SUITS[i],
                       colour: Object.values(HandsOfTime.CARD_COLOURS)[i],
                       index: "unknown",
                       description: (j+1) + " OF " + HandsOfTime.CARD_SUITS[i] + "S"
                   })
